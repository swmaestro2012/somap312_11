...
void CppCheck::checkFunctionUsage()
{
    ...
    if (_settings.isEnabled("unusedFunction") && _settings._jobs == 1) {
		...
		//편집
		_makeFunctionFunctionTable.check(this);
		_makeClassFunctionFunctionsTable.check(this);
		_makeClassVariableFunctionsTable.check(this);
...
void CppCheck::checkFile(const std::string &code, const char FileName[])
{
    ...
    try {
		...
		//편집
		_makeFunctionFunctionTable.parseTokens(_tokenizer);
		_makeClassFunctionFunctionsTable.parseTokens(_tokenizer);
		_makeClassVariableFunctionsTable.parseTokens(_tokenizer);
